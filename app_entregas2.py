...
st.markdown("""
    ...
    .stDownloadButton>button {background-color: #28a745 !important; color: white !important; width: 100% !important; height: 60px !important; border-radius: 12px !important;}
    </style>
""", unsafe_allow_html=True)

# =================================================================
# 2. SISTEMA DE MEMÓRIA (ANTI-FALHA)
# =================================================================
...
if st.session_state.pontos_carregados:
    ...
    # Define centro do mapa (Sem loop infinito)
    # Prioridade: 1. Quadra selecionada | 2. Próxima sugerida | 3. Última posição
    if st.session_state.ponto_clicado:
        map_center = st.session_state.pontos_carregados[st.session_state.ponto_clicado]
    elif proximo_ideal:
        map_center = st.session_state.pontos_carregados[proximo_ideal]
    else:
        map_center = st.session_state.ultima_pos
    ...
    # Renderiza o mapa filtrando apenas o clique no popup (Evita loops de center/zoom)
    map_data = st_folium(m, use_container_width=True, height=450, returned_objects=["last_object_clicked_popup"])
        ...
        with c2:
            if nome_sel not in st.session_state.entregues:
                if st.button("✅ FEITO"):
                    ...
                    st.session_state.ponto_clicado = None # Limpa seleção após concluir
                    salvar_progresso()
                    st.rerun()
            else: st.success("Entregue! ✔️")

    if proximo_ideal: st.warning(f"💡 Sugestão Próxima: **{proximo_ideal}**")
    st.progress(len(st.session_state.entregues)/len(st.session_state.pontos_carregados) if st.session_state.pontos_carregados else 0)
