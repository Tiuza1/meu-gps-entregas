...
# --- CSS PARA TELA CHEIA E BOTÕES GRANDES (MOBILE) ---
...
# =================================================================
# 2. SISTEMA DE MEMÓRIA E PERSISTÊNCIA (ANTI-FALHA)
# =================================================================
...
# Tenta carregar ao iniciar o app
...
with st.expander("⚙️ CONFIGURAR / RELATÓRIO", expanded=not st.session_state.pontos_carregados):
    ...
    try:
        ...
        banco_total = {}
        st.error("Erro ao ler arquivo das quadras.")

    base_input = st.text_input("📍 Ponto de Início:", "Luziânia, GO")
...
if st.session_state.pontos_carregados:
    ...
    # Define onde o mapa abre (memória do usuário)
    map_pos = st.session_state.map_center if st.session_state.map_center else st.session_state.ultima_pos
    
    m = folium.Map(location=map_pos, zoom_start=st.session_state.map_zoom)
    # auto_start=False e fly_to=False para o mapa NÃO pular sozinho!
    ...
    for nome, coords in st.session_state.pontos_carregados.items():
        ...
        opacidade = "0.4" if status else "1.0"
        ...
        icon_html = f"""<div style="background-color:{cor}; width:{tamanho}; height:{tamanho}; border-radius:50%; display:flex; 
                        ...
                        border:2px solid white; box-shadow: 2px 2px 8px rgba(0,0,0,0.4); opacity:{opacidade};">{txt}</div>"""
        
        folium.Marker(location=coords, popup=nome, icon=folium.DivIcon(html=icon_html)).add_to(m)

    # Renderiza o mapa e captura movimentos do dedo
    map_data = st_folium(m, use_container_width=True, height=450, key="mapa_v1")

    # Salva posição atual do mapa para o próximo recarregamento não pular
    ...
    st.info("Configure as quadras acima para abrir o mapa.")
