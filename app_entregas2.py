...
st.markdown("""
    ...
    /* Esconde a opção 'Select All' e o título da caixa de busca */
...
with c1:
    busca = st.selectbox("Busca", options=["(Digite a Quadra)"] + list(banco_total.keys()), label_visibility="collapsed")
with c2:
    if st.button("➕"):
        if busca and busca != "(Digite a Quadra)":
...
# =================================================================
# 2. AGRUPAMENTO DE PACOTES E LÓGICA DE SUGESTÃO
# =================================================================
quadras_agrupadas = {}
...
elif st.session_state.lista_pacotes and proximo_ideal:
    st.info(f"💡 Sugestão: Vá para **{proximo_ideal}**")
elif not st.session_state.lista_pacotes:
